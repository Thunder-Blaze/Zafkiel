<script lang="ts">
	...
	const statuses = $derived<{ value: MediaListStatus; label: string; icon: string; color: string }[]>([
		{ value: 'CURRENT',   label: isManga ? 'Reading'     : 'Watching',   icon: 'solar:play-circle-bold',    color: 'bg-green-500/20 text-green-400 border-green-500/40'   },
		{ value: 'PLANNING',  label: 'Planning',                              icon: 'solar:bookmark-bold',        color: 'bg-blue-500/20  text-blue-400  border-blue-500/40'    },
		{ value: 'COMPLETED', label: 'Completed',                             icon: 'solar:check-circle-bold',    color: 'bg-primary/20   text-primary   border-primary/40'     },
		{ value: 'PAUSED',    label: 'Paused',                                icon: 'solar:pause-circle-bold',    color: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/40'},
		{ value: 'DROPPED',   label: 'Dropped',                               icon: 'solar:close-circle-bold',    color: 'bg-red-500/20   text-red-400   border-red-500/40'     },
		{ value: 'REPEATING', label: isManga ? 'Re-reading'  : 'Rewatching',  icon: 'solar:refresh-circle-bold',  color: 'bg-purple-500/20 text-purple-400 border-purple-500/40'},
	]);

	const scoreLabel = $derived(() => {
		if (score === 0) return 'No score';
		const labels = ['', '1 – Appalling', '2 – Horrible', '3 – Very Bad', '4 – Bad', '5 – Average',
	...
	<Dialog.Content class="max-w-lg gap-0 overflow-hidden p-0">
		...
		<div class="relative h-28 w-full overflow-hidden bg-muted">
			...
			<div class="absolute inset-x-0 bottom-0 flex items-end gap-3 px-5 pb-3">
				...
				<div class="min-w-0">
					<h2 class="line-clamp-2 text-base font-bold leading-snug">{mediaTitle}</h2>
		...
		<div class="flex max-h-[70vh] flex-col gap-5 overflow-y-auto px-5 py-5">
					...
					{#each statuses as s}
						<button
							...
							class="flex items-center gap-1.5 rounded-md border px-2.5 py-2 text-xs font-medium transition-all hover:scale-[1.02]
								{selectedStatus === s.value ? s.color : 'border-border bg-background text-muted-foreground hover:bg-accent'}"
						>
			...
			<div>
				<div class="mb-2 flex items-center justify-between">
					<p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Score</p>
					<span class="text-xs font-medium text-primary">{scoreLabel()}</span>
				</div>
				<div class="flex items-center gap-3">
					<button type="button" onclick={() => (score = Math.max(0, score - 1))}
					...
					<div class="flex flex-1 items-center gap-1">
						{#each Array.from({ length: 10 }, (_, i) => i + 1) as n}
							<button type="button" onclick={() => (score = score === n ? 0 : n)}
								class="h-2 flex-1 rounded-full transition-all {n <= score ? 'bg-primary' : 'bg-border hover:bg-muted-foreground/40'}"
								title={n.toString()}></button>
						{/each}
					</div>
					<button type="button" onclick={() => (score = Math.min(10, score + 1))}
						class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border bg-background hover:bg-accent"
						disabled={score >= 10}>
			...
			<div class="grid {isManga ? 'grid-cols-3' : 'grid-cols-2'} gap-3">
				...
				<div>
					...
					<div class="flex items-center gap-1">
						<button type="button" onclick={() => (progress = Math.max(0, progress - 1))}
						...
						<input type="number" min="0" max={totalEpisodes ?? totalChapters ?? undefined}
							bind:value={progress}
						...
						<button type="button"
							onclick={() => (progress = Math.min((totalEpisodes ?? totalChapters ?? Infinity), progress + 1))}
							class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-background hover:bg-accent"
							disabled={(totalEpisodes ?? totalChapters) != null && progress >= (totalEpisodes ?? totalChapters ?? 0)}>
							<Icon icon="solar:add-circle-linear" class="size-3.5" />
						</button>
					</div>
					{#if totalEpisodes ?? totalChapters}
						<p class="mt-1 text-right text-[10px] text-muted-foreground">of {totalEpisodes ?? totalChapters}</p>
					{/if}
				</div>
				...
				{#if isManga}
					<div>
						<p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Volume</p>
						<div class="flex items-center gap-1">
							<button type="button" onclick={() => (progressVolumes = Math.max(0, progressVolumes - 1))}
								class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-background hover:bg-accent"
								disabled={progressVolumes <= 0}>
								<Icon icon="solar:minus-circle-linear" class="size-3.5" />
							</button>
							<input type="number" min="0" max={totalVolumes ?? undefined}
								bind:value={progressVolumes}
								class="h-8 w-full min-w-0 rounded-md border bg-background px-2 text-center text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
							<button type="button"
								onclick={() => (progressVolumes = Math.min(totalVolumes ?? Infinity, progressVolumes + 1))}
								class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-background hover:bg-accent"
				...
				<div>
					...
					<div class="flex items-center gap-1">
						<button type="button" onclick={() => (repeat = Math.max(0, repeat - 1))}
							class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-background hover:bg-accent"
							disabled={repeat <= 0}>
							<Icon icon="solar:minus-circle-linear" class="size-3.5" />
						</button>
						<input type="number" min="0" bind:value={repeat}
							class="h-8 w-full min-w-0 rounded-md border bg-background px-2 text-center text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
						<button type="button" onclick={() => (repeat = repeat + 1)}
							class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-background hover:bg-accent">
							<Icon icon="solar:add-circle-linear" class="size-3.5" />
						</button>
					</div>
			...
			<div class="grid grid-cols-2 gap-3">
				<div>
					<p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Start Date</p>
					<div class="relative">
						<input type="date" bind:value={startedAt}
							class="h-9 w-full rounded-md border bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 [color-scheme:dark]" />
						{#if startedAt}
							<button type="button" onclick={() => (startedAt = '')}
				...
				<div>
					<p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Finish Date</p>
					<div class="relative">
						<input type="date" bind:value={completedAt}
							class="h-9 w-full rounded-md border bg-background px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 [color-scheme:dark]" />
						{#if completedAt}
							<button type="button" onclick={() => (completedAt = '')}
								class="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
								<Icon icon="solar:close-circle-linear" class="size-3.5" />
							</button>
						{/if}
			...
			<!-- Notes ──────────────────────────────────────────────────────── -->
			<div>
				<p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Notes</p>
				<textarea bind:value={notes} placeholder="Your private notes…" rows={2}
					class="w-full resize-none rounded-md border bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"></textarea>
			</div>

			<!-- Toggles ────────────────────────────────────────────────────── -->
			<div class="flex items-center gap-5">
				<button type="button" onclick={() => (hiddenFromStatus = !hiddenFromStatus)}
					class="flex items-center gap-2 text-xs transition-colors {hiddenFromStatus ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}">
					<div class="flex h-4 w-4 items-center justify-center rounded border {hiddenFromStatus ? 'border-primary bg-primary' : 'border-border bg-background'}">
						{#if hiddenFromStatus}<Icon icon="solar:check-bold" class="size-2.5 text-primary-foreground" />{/if}
					</div>
					Hide from status lists
				</button>
				<button type="button" onclick={() => (isPrivate = !isPrivate)}
					class="flex items-center gap-2 text-xs transition-colors {isPrivate ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}">
					<div class="flex h-4 w-4 items-center justify-center rounded border {isPrivate ? 'border-primary bg-primary' : 'border-border bg-background'}">
						{#if isPrivate}<Icon icon="solar:check-bold" class="size-2.5 text-primary-foreground" />{/if}
					</div>
					Private
				</button>
			</div>
			...
			<div class="flex items-center gap-2 border-t pt-4">
				{#if existing?.id}
					<Button variant="destructive" size="sm" class="mr-auto" onclick={handleRemove} disabled={isPending}>
						<Icon icon="solar:trash-bin-2-linear" class="mr-1.5 size-4" />Remove
					</Button>
				{/if}
				<Button variant="outline" size="sm" onclick={() => { open = false; onClose?.(); }} disabled={isPending}>
					Cancel
				</Button>
