...
<button
	class="group relative flex w-full cursor-pointer flex-col overflow-hidden rounded-lg border bg-card text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
	onclick={() => goto(href)}
>
	<!-- Cover image -->
	<div class="relative aspect-2/3 w-full overflow-hidden bg-muted">
		...
		{#if countdown}
			<div
				class="absolute inset-x-0 bottom-0 bg-linear-to-t from-black/80 via-black/40 to-transparent px-2 pt-6 pb-2"
			>
	...
	<div class="flex items-start justify-between gap-2 p-2">
		<p
			class="min-w-0 flex-1 truncate text-[11px] leading-tight font-medium text-foreground group-hover:text-primary"
		>
			{title}
		</p>
		{#if scoreLabel !== 'N/A'}
