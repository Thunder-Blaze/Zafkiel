...
<div class="flex flex-col gap-6 px-6 py-8">
	...
	{:else}
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
			{#each recs() as rec}
				<div class="flex flex-col overflow-hidden rounded-xl border border-border/50 bg-card transition-colors hover:border-primary/30">
					<!-- Side-by-side covers -->
					<div class="flex h-36 overflow-hidden">
						...
						<div class="relative shrink-0">
							<div class="absolute inset-0 flex items-center justify-center bg-background/80 z-10 w-7">
						...
						<button
							class="w-1/2 overflow-hidden"
							onclick={() => rec.mediaRecommendation?.id && goto(`/anime/${rec.mediaRecommendation.id}`)}
						>
					...
					<!-- Titles row -->
					<div class="flex gap-1 px-2 pt-2 text-[10px] font-medium leading-snug">
						<span class="w-1/2 truncate text-center">{mediaTitle(rec.media)}</span>
						<span class="w-1/2 truncate text-center text-primary">{mediaTitle(rec.mediaRecommendation)}</span>
					</div>

					<!-- Rating row -->
					<div class="flex items-center justify-between gap-2 px-3 py-2.5 mt-auto border-t border-border/30">
						{#if rec.user?.name}
