...
fn get_themes_dir() -> Result<PathBuf, String> {
    ...
    if let Ok(current_exe) = std::env::current_exe() {
        ...
        if let Some(exe_parent) = current_exe
            .parent()
            .and_then(|p| p.parent())
            .and_then(|p| p.parent())
            .and_then(|p| p.parent())
    ...
    println!(
        "[Theme Commands] Trying {} possible paths:",
        possible_paths.len()
    );
        ...
        if theme_json_path.exists() {
            if let Ok(content) = fs::read_to_string(&theme_json_path) {
                match serde_json::from_str::<ThemeMetadata>(&content) {
                    Ok(metadata) => {
                        println!(
                            "[Theme Commands]   ✓ Loaded theme: {} (id: {})",
                    ...
                    Err(e) => {
                        println!(
                            "[Theme Commands]   ✗ Failed to parse theme.json for {}: {}",
                            theme_name, e
                        );
                    }
                }
                ...
                println!(
                    "[Theme Commands]   ✗ Failed to read theme.json for {}",
                    theme_name
                );
            }
        } else {
            println!(
                "[Theme Commands]   ✗ No theme.json found for {}",
                theme_name
            );
        }
    }
