...
export function unpackEval(packedString: string): string {
  ...
  const p = (p: string, a: number, c: number, k: string[], e: (n: number) => string, d: Record<string, string>) => {
...
// index.ts
import type {
  SourceExtension, SearchResult, AnimeDetails, Episode,
  Page, StreamSource, ResolvedStream, CookieEntry
...
export class AnimePaheExtension implements SourceExtension {
  ...
  async getAnimeDetails(id: string): Promise<AnimeDetails> {
    ...
    const title = html.match(/<span style="user-select:text">(.+?)<\/span>/)?.[1]
      ?? html.match(/<meta property="og:title" content="(.+?)">/)?.[1]
      ?? id;

    const coverUrl = html.match(/<a href="(https:\/\/i\.animepahe\.si\/posters.+?)"/)?.[1]
      ?? html.match(/<meta property="og:image" content="(.+?)">/)?.[1];

    const description = html.match(/<div class="anime-synopsis">(.+?)<\/div>/)?.[1]
  ...
  async getEpisodes(animeSessionId: string, page = 1): Promise<Page<Episode>> {
    ...
    const episodes: Episode[] = (data.data ?? []).map((ep) => ({
      ...
      thumbnailUrl: ep.snapshot
        ? `https://i.animepahe.si/snapshots/${ep.snapshot}`
        : undefined,
