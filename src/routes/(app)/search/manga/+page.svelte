<script lang="ts">
	...
	import { useTrendingManga, usePopularManga, useBrowseMedia } from '$lib/hooks/useAnilist.svelte';
						...
						>
							{#if currentHero.format}
								<div class="text-[11px] font-bold tracking-widest text-primary uppercase">
									{currentHero.format.replace(/_/g, ' ')}
								</div>
							{/if}
							...
							<div class="flex flex-wrap items-center gap-2 text-sm">
								...
								{/if}
								{#if currentHero.chapters}<span class="text-xs text-muted-foreground"
										>{currentHero.chapters} ch</span
									>{/if}
								{#if currentHero.volumes}<span class="text-xs text-muted-foreground"
										>{currentHero.volumes} vol</span
									>{/if}
