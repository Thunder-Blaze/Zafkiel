<script lang="ts">
	...
	let { recommendation, sourceMediaId }: { recommendation: Recommendation; sourceMediaId?: number } = $props();
	...
	>
		...
		<div class="relative h-40 w-full overflow-hidden">
			...
			{#if (recommendation.rating ?? 0) > 0}
				<div class="absolute top-2 right-2 flex items-center gap-1 rounded-md bg-card/85 px-2 py-0.5 text-xs backdrop-blur-sm">
