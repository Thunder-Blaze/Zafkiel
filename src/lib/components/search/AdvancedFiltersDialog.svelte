		...
		(() => {
			...
			if (isAnime) {
				groups.push({
					id: 'streaming',
					title: 'Streaming On',
					category: 'Streaming On',
					tags: ANIME_STREAMING_ON,
				});
			} else {
				for (const [lang, platforms] of Object.entries(MANGA_READABLE_ON)) {
					groups.push({
						id: `readable-${lang}`,
						title: `Readable On (${lang})`,
						category: `Readable On (${lang})`,
						tags: platforms,
					});
				}
			}
...
<Dialog.Root bind:open>
	<Dialog.Content
		class="flex max-h-[90vh] w-[90vw] max-w-4xl flex-col gap-0 overflow-hidden border-border/40 bg-background/95 p-0 shadow-2xl backdrop-blur-md"
	>
		<!-- Header -->
		<div
			class="z-20 flex shrink-0 items-center justify-between border-b border-border/40 bg-background/95 px-6 py-4 shadow-sm backdrop-blur-sm"
		>
			...
			<div class="flex items-center gap-3">
				<Button
					...
					size="sm"
					onclick={reset}
					class="h-8 text-xs font-semibold hover:bg-muted/80">Reset All</Button
				>
				<Dialog.Close
					class="rounded-full p-1.5 text-muted-foreground ring-primary transition-colors outline-none hover:bg-muted hover:text-foreground focus-visible:ring-2"
				>
		...
		<!-- Virtualized Body -->
		<div class="min-h-[400px] flex-1 overflow-y-auto bg-muted/10 px-6 py-4" bind:this={scrollEl}>
							...
							<div class="mb-8 flex flex-col gap-3">
								...
								<div class="flex flex-wrap gap-2 pr-4">
									{#each group.tags as tag}
										<button
											class="rounded-lg border px-3 py-1.5 text-xs font-semibold ring-primary transition-all duration-200 outline-none focus-visible:ring-2
		...
		<!-- Footer -->
		<div
			class="z-20 flex shrink-0 items-center justify-end border-t border-border/40 bg-background/95 px-6 py-4 shadow-sm backdrop-blur-sm"
		>
			<Dialog.Close>
				<Button
					class="font-semibold shadow-md transition-transform hover:scale-[1.02] active:scale-95"
				>
