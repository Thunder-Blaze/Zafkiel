...
use raw_window_handle::{HasWindowHandle, HasDisplayHandle, RawDisplayHandle, RawWindowHandle};
...
pub struct MpvApi {
    ...
    pub set_property_string: unsafe extern "C" fn(*mut c_void, *const c_char, *const c_char) -> c_int,
    ...
    // Render API
    pub render_context_create: unsafe extern "C" fn(*mut *mut c_void, *mut c_void, *mut MpvRenderParam) -> c_int,
    pub render_context_set_update_callback: unsafe extern "C" fn(*mut c_void, Option<unsafe extern "C" fn(*mut c_void)>, *mut c_void),
...
impl MpvApi {
    pub unsafe fn load() -> Result<Self, String> {
        ...
        let set_option_string = *unsafe { lib.get(b"mpv_set_option_string\0").map_err(|e| e.to_string())? };
        ...
        let set_property_string = *unsafe { lib.get(b"mpv_set_property_string\0").map_err(|e| e.to_string())? };
        let get_property_string = *unsafe { lib.get(b"mpv_get_property_string\0").map_err(|e| e.to_string())? };
        let terminate_destroy = *unsafe { lib.get(b"mpv_terminate_destroy\0").map_err(|e| e.to_string())? };
        ...
        let observe_property = *unsafe { lib.get(b"mpv_observe_property\0").map_err(|e| e.to_string())? };
        
        let render_context_create = *unsafe { lib.get(b"mpv_render_context_create\0").map_err(|e| e.to_string())? };
        let render_context_set_update_callback = *unsafe { lib.get(b"mpv_render_context_set_update_callback\0").map_err(|e| e.to_string())? };
        let render_context_update = *unsafe { lib.get(b"mpv_render_context_update\0").map_err(|e| e.to_string())? };
        let render_context_render = *unsafe { lib.get(b"mpv_render_context_render\0").map_err(|e| e.to_string())? };
        let render_context_free = *unsafe { lib.get(b"mpv_render_context_free\0").map_err(|e| e.to_string())? };
        let free_node_contents = *unsafe { lib.get(b"mpv_free_node_contents\0").map_err(|e| e.to_string())? };
    ...
    match node.format {
        ...
        1 | 2 => { // MPV_FORMAT_STRING | MPV_FORMAT_OSD_STRING
            ...
            if string_ptr.is_null() {
                ...
                Value::String(unsafe { CStr::from_ptr(string_ptr) }.to_string_lossy().into_owned())
            }
        }
...
pub unsafe fn create_headless_context(
    ...
    let surface_attributes = glutin::surface::SurfaceAttributesBuilder::<glutin::surface::PbufferSurface>::new()
        .build(
    ...
    Ok(GlContext { display, surface, context })
}
...
pub struct ActiveMpvPlayer {
    ...
    pub channel: Arc<Mutex<Option<Channel<Vec<u8>>>>>,
...
pub fn init(
    ...
    log::info!("[MPV Render] Initializing player for window: {}", windowLabel);
    
    let api = state.api.as_ref().ok_or_else(|| "libmpv API not loaded".to_string())?.clone();
    ...
    let window = app.get_webview_window(&windowLabel)
        .ok_or_else(|| format!("Window not found: {}", windowLabel))?;
        
    let display_handle_guard = window.display_handle()
        .map_err(|e| format!("Failed to get display handle: {:?}", e))?;
    let window_handle_guard = window.window_handle()
    ...
    let channel_ptr: Arc<Mutex<Option<Channel<Vec<u8>>>>> = Arc::new(Mutex::new(None));
    ...
    if let Some(observed) = mpvConfig.get("observedProperties").and_then(|o| o.as_object()) {
        for (k, v) in observed {
            let name_c = CString::new(k.as_str()).unwrap();
    ...
    let gl_context = unsafe { create_headless_context(display_handle, window_handle) }
        .map_err(|e| {
            unsafe { (api.terminate_destroy)(handle); }
    ...
    let render_thread = thread::spawn(move || {
        ...
        let current_context = match gl_context.context.make_current(&gl_context.surface) {
            Ok(c) => c,
            Err(e) => {
                log::error!("[MPV Render Thread] Failed to make GL context current: {:?}", e);
                return;
            }
        };
        ...
        let mut params = if !wl_display_ptr.is_null() {
            [
                ...
                init_params_param,
                wl_display_param,
                MpvRenderParam { type_: 0, data: ptr::null_mut() },
            ]
        } else {
            [
                api_type_param,
                init_params_param,
                MpvRenderParam { type_: 0, data: ptr::null_mut() },
                MpvRenderParam { type_: 0, data: ptr::null_mut() }, // padding slot
            ]
        };
        ...
        if err < 0 {
            log::error!("[MPV Render Thread] Failed to create render context: {}", err);
            return;
        }
        ...
        loop {
            ...
            match signal_rx.recv_timeout(Duration::from_millis(8)) {
                Ok(()) => {
                    ...
                    let mut h: i64 = 0;
                    unsafe {
                        (api_clone1.get_property)(handle, width_name.as_ptr(), 4, &mut w as *mut _ as *mut c_void);
                        (api_clone1.get_property)(handle, height_name.as_ptr(), 4, &mut h as *mut _ as *mut c_void);
                    }
                    ...
                    if w > 0 && h > 0 {
                        ...
                        if w != current_w || h != current_h {
                            ...
                            unsafe {
                                ...
                                if status != gl::FRAMEBUFFER_COMPLETE {
                                    log::info!("[MPV Render Thread] Framebuffer is not complete: {}", status);
                                }
                            }
                        }
                        ...
                        unsafe {
                            let render_err = (api_clone1.render_context_render)(render_ctx, render_params.as_mut_ptr());
                            if render_err >= 0 {
                                // Read pixels
                                ...
                                log::info!("[MPV Render Thread] mpv_render_context_render failed: {}", render_err);
                            }
                        }
                    }
                }
        ...
        unsafe {
            ...
            drop(Box::from_raw(signal_tx_box as *mut std::sync::mpsc::SyncSender<()>));
        ...
        loop {
            ...
            match event.event_id {
                8 => { // MPV_EVENT_FILE_LOADED
                    payload.insert("event".to_string(), Value::String("file-loaded".to_string()));
                    let _ = app_clone1.emit(&event_name, payload);
                }
                21 => { // MPV_EVENT_PLAYBACK_RESTART
                    payload.insert("event".to_string(), Value::String("playback-restart".to_string()));
                    let _ = app_clone1.emit(&event_name, payload);
                }
...
pub fn destroy(
    ...
    if let Some(mut player) = lock.take() {
        ...
        let api = state.api.as_ref().ok_or_else(|| "libmpv API not loaded".to_string())?;
        unsafe {
...
pub fn command(
    ...
    let player = lock.as_ref().ok_or_else(|| "Player not initialized".to_string())?;
    let api = state.api.as_ref().ok_or_else(|| "libmpv API not loaded".to_string())?;
    
    log::info!("[MPV Render] Sending command '{}' to window: {}", name, windowLabel);
    ...
    let err = unsafe {
        (api.command)(player.handle, ptrs.as_mut_ptr())
    };
    if err < 0 {
...
pub fn set_property(
    ...
    log::info!("[MPV Render] Setting property '{}' = {} on window: {}", name, value, windowLabel);
    ...
    let err = unsafe {
        (api.set_property_string)(player.handle, name_c.as_ptr(), val_c.as_ptr())
    };
    if err < 0 {
...
pub fn get_property(
    ...
    let err = unsafe {
        (api.get_property)(player.handle, name_c.as_ptr(), 6, &mut node as *mut _ as *mut c_void)
    };
    if err < 0 {
        ...
        unsafe { (api.free)(val_ptr as *mut c_void); }
