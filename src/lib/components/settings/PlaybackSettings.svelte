<script lang="ts">
	...
	onMount(async () => {
		try {
			const result = await invoke<{ success: boolean; data?: { external_player_path?: string | null } }>('get_player_config');
		...
		<CardContent class="space-y-6">
			...
			<div class="space-y-3 rounded-lg border border-border/50 bg-foreground/5 p-4">
				<div class="space-y-0.5">
					...
					<p class="text-sm text-foreground/70">
						...
						<code class="rounded bg-muted px-1 py-0.5 text-xs">C:\tools\mpv\mpv.exe</code>).
						Leave empty to use <code class="rounded bg-muted px-1 py-0.5 text-xs">mpv</code> from system PATH.
					</p>
				</div>
				<div class="flex gap-2">
					<Input
						...
						onkeydown={(e: KeyboardEvent) => { if (e.key === 'Enter') saveExternalPlayerPath(); }}
					/>
					<Button
						variant="outline"
						size="sm"
			...
			>
				<div class="space-y-0.5">
					...
					<p class="text-sm text-foreground/70">Automatically pick the best stream based on preference</p>
				</div>
				<Switch 
