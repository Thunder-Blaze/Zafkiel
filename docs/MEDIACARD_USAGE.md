...
<div class="w-64 space-y-1.5">
	<h3 class="mb-2 text-sm font-semibold">Continue Watching</h3>
...
<div class="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
	{#each items as item, index}
		...
		{@const position = index % cols === 0 ? 'left' : (index + 1) % cols === 0 ? 'right' : 'center'}
		<MediaCard mediaData={item} variant="grid" size="md" {position} />
	{/each}
</div>
```
...
<MediaCard {mediaData} onStatusChange={(status) => handleStatusChange(mediaData.id, status)} />
```
