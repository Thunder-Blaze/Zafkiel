<script lang="ts">
	...
	import { useUserMangaList } from '$lib/hooks/useAnilist.svelte';
	...
	const mangaQuery = $derived(useUserMangaList(username, activeStatus, currentPage, 50));
	const isLoading = $derived(mangaQuery.isLoading);
	const entries = $derived((mangaQuery.data?.data?.data ?? []) as MediaList[]);
	const pageInfo = $derived(mangaQuery.data?.data?.pageInfo);
	...
	const statusOptions: { label: string; value: MediaListStatus | undefined }[] = [
		...
		{ label: 'Rereading', value: 'REPEATING' },
	];
...
<svelte:head>
	<title>{username}'s Manga List — Zafkiel</title>
</svelte:head>
...
<div class="container mx-auto max-w-7xl px-4 py-6">
	...
	<div use:gsapReveal class="mb-6 flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold">{username}'s Manga List</h1>
		</div>
		<div class="flex items-center gap-3">
			<!-- Switch to anime list -->
			<Button variant="outline" size="sm" onclick={() => goto(`/user/${username}/animelist`)}>
				<Icon icon="solar:play-circle-bold-duotone" class="mr-1.5 size-4" />
				Anime List
			</Button>
			<!-- View mode toggle -->
	...
	{#if isLoading}
		...
		{/if}
	{:else if mangaQuery.error || (mangaQuery.data && !mangaQuery.data.success)}
		<div class="flex min-h-[300px] flex-col items-center justify-center gap-4 text-center">
			...
			<p class="text-muted-foreground">
				{mangaQuery.error?.message || mangaQuery.data?.error || 'Failed to load manga list'}
			</p>
	...
	{:else if entries.length === 0}
		<div class="flex min-h-[300px] flex-col items-center justify-center gap-3 text-center">
			<Icon icon="solar:book-2-bold-duotone" class="h-16 w-16 text-muted-foreground" />
			<h3 class="text-lg font-semibold">No manga found</h3>
			<p class="text-sm text-muted-foreground">
				{username} hasn't added any manga in this category yet.
			</p>
		</div>
	{:else}
		{#if viewMode === 'list'}
