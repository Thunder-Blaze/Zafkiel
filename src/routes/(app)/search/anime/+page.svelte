<script lang="ts">
	...
	function prevHero() {
		...
		if (heroTimer) { clearInterval(heroTimer); heroTimer = setInterval(() => { heroDirection = 'right'; heroIdx = (heroIdx + 1) % heroes.length; }, 6000); }
	}
	function nextHero() {
		heroDirection = 'right';
		heroIdx = (heroIdx + 1) % heroes.length;
			...
			<div class="relative grid h-full grid-cols-[1fr_auto] items-end gap-4 px-8 pb-10 lg:px-14">
				...
				{#key currentHero.id}
					...
					>
						...
						{/if}
						<h1 class="line-clamp-1 text-3xl font-bold leading-tight drop-shadow-lg md:text-4xl">
						...
						<div class="flex flex-wrap items-center gap-2 text-sm">
							...
							{/if}
							{#if currentHero.format}<span class="rounded bg-foreground/10 px-2 py-0.5 text-xs font-medium">{currentHero.format.replace(/_/g, ' ')}</span>{/if}
							{#if currentHero.episodes}<span class="text-xs text-muted-foreground">{currentHero.episodes} eps</span>{/if}
	...
	<div class="flex flex-col gap-10 px-6 py-8">
		...
		<section id="trending">
			<div class="mb-4 flex items-center justify-between">
				...
				<Button variant="ghost" size="sm" onclick={() => goto('/browse/anime?sort=TRENDING_DESC')} class="gap-1 text-xs text-muted-foreground">
		...
		<section id="popular">
			<div class="mb-4 flex items-center justify-between">
				...
				<Button variant="ghost" size="sm" onclick={() => goto('/browse/anime?sort=POPULARITY_DESC')} class="gap-1 text-xs text-muted-foreground">
					View All <Icon icon="solar:arrow-right-linear" class="h-3.5 w-3.5" />
				</Button>
			</div>
		...
		<section id="movies">
			<div class="mb-4 flex items-center justify-between">
				...
				<Button variant="ghost" size="sm" onclick={() => goto('/browse/anime?format=MOVIE&sort=SCORE_DESC')} class="gap-1 text-xs text-muted-foreground">
					View All <Icon icon="solar:arrow-right-linear" class="h-3.5 w-3.5" />
				</Button>
			</div>
